...
class UserController extends Controller {
	...
	public function followers($page = 1) {
		...
		$articles = Article::whereIn('user_id', $followerArray)->where('published', '=', '1')->orderBy('last_updated', 'asc')->skip($page - 1)->take($resultsPerPage)->get();
	...
	public function drafts($page = 1) {
		...
		$articles = Article::where('user_id', '=', Auth::user()->id)->where('published', '=', '0')->orderBy('last_updated', 'asc')->skip($page - 1)->take($resultsPerPage)->get();
