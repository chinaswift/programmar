...
class HomeController extends Controller {
	...
	public function all($page = 1)
	{
		...
		$articles = Article::where('published', '=', '1')->orderBy('last_updated', 'desc')->skip(($page - 1) * $resultsPerPage)->take($resultsPerPage)->get();
	...
	public function popular($page = 1) {
		...
		$articles = Article::where('published', '=', '1')->where('slug', '>', $lastDay)->where('slug', '<', $nextDay)->skip(($page - 1) * $resultsPerPage)->take($resultsPerPage)->get();
		foreach ($articles as $article) {
